...
namespace Explorator
{
    public class PercentageBar : Control
    {
        ...
        [Category("Appearance"), Description("Determine wether to automatically show percentage in bytes only.")]
        [Browsable(true)]
        [DefaultValue(true)]
        public bool AutomaticallyFormatPercentageToBytes
        {
            get => _AutoBytes;
            set => _AutoBytes = value;
        }
            ...
            using (GraphicsPath gp = new GraphicsPath())
            {
                ...
                if (_ShowPercentage | _ShowText)
                {
                    ...
                    if (_ShowPercentage)
                    {
                        ...
                        if(_AutoBytes)
                            perc += string.Format(" ({0} / {1})", Utils.GetFileSize(_currentBytes),
                                Utils.GetFileSize(MaximumBytes));
                    }
