...
namespace Core.Utils.CRC
{
    ...
    {
        static SlicingBy8 slicer = new SlicingBy8();
        ...
        public static UInt32 Compute(byte[] data)
        {
            ...
            var r = data.Aggregate(0xffffffff, (current, t) => (current >> 8) ^ CRCTable[t ^ (current & 0x000000FF)]);
            //Parallel.For(0, data.Length, i => { r = (r >> 8) ^ CRCTable[data[i] ^ (r & 0x000000FF)]; }
            //);
            return ~r;
        }

        public static UInt32 QuickCompute(byte[] data, SlicingType sType)
        {
            slicer.InitCrc();
            ...
            using (var mStream = new MemoryStream(data))
            {
                switch (sType)
                {
                    case SlicingType.SliceBy8:
                        return slicer.UnsafeDoSb8(mStream);
                    case SlicingType.SliceBy32:
                        return slicer.DoSb32(mStream);
                    default:
                    case SlicingType.NormalSpeed:
                        return slicer.DoSarwate(mStream);
                }
            }
        }
    }
